...
- `BinarySearch` - Поиск (требует сортировки)
- `CopyTo` - Копировать в массив
- `Contains` - Проверить наличие
- `Clear` - Очистить
- `Exists` - Есть ли такой элемент
- `Find` - Найти первый
- `FindLast` - Найти последний
- `FindAll` - Найти все
- `IndexOf` - Индекс первого
- `LastIndexOf` - Индекс последнего
- `GetRange` - Вырезать часть
...
// numbers = [1, 1, 3, 4, 5]
```
## Sort(IComparer<T> comparer) — сортировка с компаратором
```c#
List<string> words = new List<string> { "apple", "Banana", "cherry" };
words.Sort(StringComparer.OrdinalIgnoreCase);
// words = ["apple", "Banana", "cherry"] (без учёта регистра)
```
